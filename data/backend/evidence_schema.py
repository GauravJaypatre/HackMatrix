...
def build_evidence_object(
    ...
    # 4. Determine involved account_ids and scenario transaction IDs via graph traversal
    ...
    # (a) If participating in a cycle, find all cycle counterparties and cycle transactions
    if struct_dict.get("is_on_any_cycle"):
        sent_edges = edges_df[edges_df["edge_type"] == "SENT_TO"] if not edges_df.empty else pd.DataFrame()
        G_sent = nx.DiGraph()
    ...
    # (b) Multi-hop chain graph traversal (for non-cycle chains, fan-outs, and multi-hop paths)
    # Perform BFS across connected synthetic SENT_TO edges to capture all accounts in the chain
    if not edges_df.empty:
        sent_edges = edges_df[edges_df["edge_type"] == "SENT_TO"]
        synth_edges: List[Dict[str, str]] = []
        for _, r in sent_edges.iterrows():
            try:
                meta = json.loads(r["metadata"]) if pd.notna(r["metadata"]) else {}
                if meta.get("is_synthetic"):
                    synth_edges.append({
                        "source": str(r["source_id"]),
                        "target": str(r["target_id"]),
                        "transaction_id": str(meta.get("transaction_id", ""))
                    })
            except Exception:
                pass
        ...
        if synth_edges:
            ...
            for edge in synth_edges:
                u, v, tid = edge["source"], edge["target"], edge["transaction_id"]
                adj.setdefault(u, []).append((v, tid))
                adj.setdefault(v, []).append((u, tid))

            visited_nodes = set(involved_accounts)
            queue = list(involved_accounts)
            while queue:
                curr = queue.pop(0)
                for neighbor, tid in adj.get(curr, []):
                    if tid:
                        synth_tx_ids.add(tid)
                    if neighbor not in visited_nodes:
                        visited_nodes.add(neighbor)
                        involved_accounts.append(neighbor)
                        queue.append(neighbor)

    # (c) Cross-reference with injected_transactions table for scenario-level completeness
    if not inj_df.empty:
        matching_scenarios = inj_df[
            (inj_df["from_account"].astype(str).isin(involved_accounts)) |
            (inj_df["to_account"].astype(str).isin(involved_accounts))
        ]
        if not matching_scenarios.empty:
            for sc_id in matching_scenarios["scenario_id"].dropna().unique():
                sc_txs = inj_df[inj_df["scenario_id"] == sc_id]
                for _, tx_row in sc_txs.iterrows():
                    tid = str(tx_row.get("transaction_id", ""))
    ...
    # (d) Also add transactions from history
...
def demonstrate_evidence_objects():
    ...
    assert "8001E34C0" in l19_evidence["account_ids"], f"8001E34C0 missing from L19 account_ids: {l19_evidence['account_ids']}"
    assert set(l19_evidence["account_ids"]) == {"80026A5A0", "80035F420", "8001E34C0"}, f"Expected all 3 chain accounts in L19, got {l19_evidence['account_ids']}"
    l19_tl_txs = [i["details"]["transaction_id"] for i in l19_evidence["timeline"] if i["event_type"] == "transaction"]
    assert "SYN_L19_001" in l19_tl_txs and "SYN_L19_002" in l19_tl_txs, f"Expected both chain transactions in L19 timeline, got {l19_tl_txs}"
